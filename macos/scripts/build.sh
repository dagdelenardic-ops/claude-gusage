...
build_app_bundle() {
    ...
    if actool --compile "$APP_BUNDLE/Contents/Resources" \
           --platform macosx \
           --minimum-deployment-target 14.0 \
           --app-icon AppIcon \
           --output-partial-info-plist /dev/null \
           "$PROJECT_DIR/Resources/Assets.xcassets" > /dev/null 2>&1; then
        echo "==> Asset Catalog compiled successfully."
    else
        echo "==> actool failed or Xcode not installed, copying AppIcon.icns directly as fallback..."
...
create_applications_alias() {
    ...
    # A plain symlink works headlessly (locked screen / CI) and is accepted by
    # verify-release.sh. Avoids depending on Finder AppleEvents, which time out
    # when no GUI session is attached.
    ln -snf /Applications "$staging_dir/Applications"
}
...
create_dmg() {
    ...
    # create-dmg styles the window via Finder/AppleScript, which times out when
    # no GUI session is attached (locked screen, SSH, CI). Try it, but fall back
    # to a plain hdiutil image so the build always produces a working DMG.
    if [[ "${CUB_HEADLESS_DMG:-0}" == "1" ]] \
        || ! "${create_dmg_args[@]}" "$DMG_PATH" "$staging_dir" > /dev/null 2>&1; then
        echo "==> Styled DMG unavailable (no GUI session?); building plain DMG via hdiutil..."
        create_plain_dmg "$staging_dir"
    fi
...
# Finder-free DMG: drag-to-Applications still works (the staging dir already
# contains the .app and an "Applications" symlink); only the custom window
# background/icon layout is omitted.
create_plain_dmg() {
    local staging_dir="$1"
    rm -f "$DMG_PATH"
    # Detach any interstitial image a prior failed create-dmg run may have left.
