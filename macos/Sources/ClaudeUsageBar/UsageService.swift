...
class UsageService: ObservableObject {
    ...
    var pctSonnetOnly: Double { (usage?.sevenDaySonnet?.utilization ?? 0) / 100.0 }
    var pctClaudeDesign: Double { (usage?.claudeDesign?.utilization ?? 0) / 100.0 }
    var pctDailyRoutineRuns: Double { (usage?.dailyRoutineRuns?.utilization ?? 0) / 100.0 }
    var resetSonnetOnly: Date? { usage?.sevenDaySonnet?.resetsAtDate }
    var resetClaudeDesign: Date? { usage?.claudeDesign?.resetsAtDate }
    var resetDailyRoutineRuns: Date? { usage?.dailyRoutineRuns?.resetsAtDate }
    ...
    func fetchUsage() async {
        ...
        do {
            ...
            if let jsonString = String(data: data, encoding: .utf8) {
                print("DEBUG: Raw API Usage JSON: \(jsonString)")
            }
            ...
            notificationService?.checkAndNotify(
                ...
                pct7d: pct7d,
                pctExtra: pctExtra,
                pctSonnetOnly: pctSonnetOnly,
                pctClaudeDesign: pctClaudeDesign,
                scopedLimits: reconciled.scopedLimits,
                reset7d: reset7d
            )
