    ...
    var body: some View {
        Form {
            ...
            Section("Notifications") {
                Toggle("Weekly reset reminder", isOn: Binding(
                    get: { notificationService.weeklyResetReminderEnabled },
                    set: { notificationService.setWeeklyResetReminderEnabled($0) }
                ))
                Text("Alerts you about 10 hours before the weekly limit resets.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)

                Toggle("Critical model-limit alerts", isOn: Binding(
                    get: { notificationService.criticalLimitAlertsEnabled },
                    set: { notificationService.setCriticalLimitAlertsEnabled($0) }
                ))
                Text("Alerts when a model-scoped limit (e.g. Fable) hits its cap.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
        ...
        .tint(Theme.accent)
        .frame(width: 400)
        .frame(maxHeight: NSScreen.main.map { $0.visibleFrame.height - 80 } ?? 520)
        .onAppear {
            focusSettingsWindow()
        }
    }
